...
use crate::app::input::event::handler::on_viiper_event::{
    keyboard_output, steamdeck_output, xbox360_output,
};
        ...
        match viiper_event {
            ...
            ViiperEvent::DeviceOutput(output) => match output {
                ...
                DeviceOutput::Xbox360 {
                    ...
                    xbox360_output::handle(self.ctx.clone(), device_id, rumble_l, rumble_r);
                }
                DeviceOutput::Steamdeck {
                    device_id,
                    output,
                } => {
                    steamdeck_output::handle(self.ctx.clone(), device_id, output);
                }
                DeviceOutput::Keyboard { device_id, leds } => {
                    keyboard_output::handle(self.ctx.clone(), device_id, leds);
                }
            },
            //

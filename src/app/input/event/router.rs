...
impl EventRouter {
    ...
    pub fn route(&self, sdl_event: &SDL_Event) {
        ...
        if let Some(handler) = self.sdl_type_handler_map.get(&event_type) {
            handler.handle_event(&Some(RoutedEvent::SdlEvent(hl_event)), sdl_event);
            return;
        }
            ...
            && let Some(handler_event) = hl_event.as_user_event_type::<HandlerEvent>()
        {
            ...
            {
                handler.handle_event(&Some(RoutedEvent::UserEvent(handler_event)), sdl_event);
                return;
            }
            tracing::warn!(
